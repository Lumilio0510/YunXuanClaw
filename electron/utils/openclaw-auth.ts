...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // ── Update main agent name to YuanXuanClaw ─────────────────────
    // Ensure the main agent has the correct display name.
    const agentsConfig = config.agents as Record<string, unknown> | undefined;
    if (agentsConfig && typeof agentsConfig === 'object') {
      const agentsList = agentsConfig.list as Array<Record<string, unknown>> | undefined;
      if (Array.isArray(agentsList)) {
        const mainAgent = agentsList.find((agent) => agent.id === 'main');
        if (mainAgent && mainAgent.name !== 'YuanXuanClaw') {
          mainAgent.name = 'YuanXuanClaw';
          modified = true;
          console.log('[sanitize] Updated main agent name to YuanXuanClaw');
        }
      }
    }

    // ── Set default thinking level to high ───────────────────────
    // Enable thinking by default for better AI reasoning visibility.
    if (agentsConfig) {
      if (!agentsConfig.defaults || typeof agentsConfig.defaults !== 'object') {
        agentsConfig.defaults = { thinkingDefault: 'high' };
        modified = true;
        console.log('[sanitize] Set default thinking level to high');
      } else {
