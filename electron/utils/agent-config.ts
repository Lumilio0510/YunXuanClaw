...
const MAIN_AGENT_NAME = 'YuanXuanClaw';
const DEFAULT_ACCOUNT_ID = 'default';
const DEFAULT_WORKSPACE_PATH = '~/.yuanxuanclaw/workspace';
...
function getDefaultAgentDirPath(agentId: string): string {
  return `~/.yuanxuanclaw/agents/${agentId}/agent`;
}
...
function getManagedWorkspaceDirectory(agent: AgentListEntry): string | null {
  ...
  const configuredWorkspace = expandPath(agent.workspace || `~/.yuanxuanclaw/workspace-${agent.id}`);
...
async function provisionAgentFilesystem(
  ...
  const targetWorkspace = expandPath(agent.workspace || `~/.yuanxuanclaw/workspace-${agent.id}`);
...
async function buildSnapshotFromConfig(config: AgentConfigDocument, preloadedChannels?: string[]): Promise<AgentsSnapshot> {
  ...
  const agents: AgentSummary[] = entries.map((entry) => {
    ...
    return {
      ...
      inheritedModel,
      workspace: entry.workspace || (entry.id === MAIN_AGENT_ID ? getDefaultWorkspacePath(config) : `~/.yuanxuanclaw/workspace-${entry.id}`),
...
export async function createAgent(
  ...
  return withConfigLock(async () => {
    ...
    const newAgent: AgentListEntry = {
      id: nextId,
      name: normalizedName,
      workspace: `~/.yuanxuanclaw/workspace-${nextId}`,
...
export async function resolveAgentIdFromChannel(
  channelType: string,
  accountId?: string,
): Promise<string | null> {
  ...
  if (accountId?.trim()) {
    const accountKey = `${normalizedChannel}:${accountId.trim()}`;
    const accountOwner = snapshot.channelAccountOwners[accountKey];
    if (accountOwner) {
      return accountOwner;
    }
  }

  const channelOwner = snapshot.channelOwners[normalizedChannel];
  if (channelOwner) {
    return channelOwner;
  }

  return snapshot.defaultAgentId || null;
}
