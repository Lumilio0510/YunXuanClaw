 ...
 * Default: ~/.yuanxuanclaw
 * Can be overridden via OPENCLAW_CONFIG_DIR environment variable for isolation
 */
export function getOpenClawConfigDir(): string {
  const envConfigDir = process.env.OPENCLAW_CONFIG_DIR?.trim();
  if (envConfigDir) {
    return expandPath(envConfigDir);
  }
  return join(homedir(), '.yuanxuanclaw'); // 使用隔离的配置目录
}
